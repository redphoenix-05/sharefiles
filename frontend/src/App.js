...
function App() {
  ...
  const [adminToken, setAdminToken] = useState(() => localStorage.getItem('sharefiles-admin-token') || '');

  const handleTitleClick = () => {
    const nextCount = adminTapCount + 1;
    if (nextCount >= 5) {
      setAdminTapCount(0);
      setActiveView(adminToken ? 'admin-dashboard' : 'admin-login');
      return;
    }

    setAdminTapCount(nextCount);
    window.clearTimeout(window.__sharefilesAdminTapTimer);
    window.__sharefilesAdminTapTimer = window.setTimeout(() => {
      setAdminTapCount(0);
    }, 1500);
  };

  const handleAdminLoginSuccess = (token) => {
    localStorage.setItem('sharefiles-admin-token', token);
    setAdminToken(token);
    setActiveView('admin-dashboard');
  };

  const handleAdminLogout = () => {
    localStorage.removeItem('sharefiles-admin-token');
    setAdminToken('');
    setActiveView('main');
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-100">
      <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="mb-8 text-center sm:mb-10 lg:mb-12">
          <h1
            className="mb-3 cursor-pointer text-4xl font-bold tracking-tight text-gray-800 sm:text-5xl"
